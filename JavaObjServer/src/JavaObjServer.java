...
public class JavaObjServer extends JFrame {
	...
	private Vector<UserService> UserVec = new Vector(); // Vector to store connected users
...
//				Login();
				...
				if (UserVec != null) {
					...
					try {
						...
						for (UserService userService: UserVec) {
							outToClient = new BufferedOutputStream(userService.client_socket.getOutputStream());
		...
		public void sendimoji(Object ob) {
...
//					msg = msg.trim(); // 앞뒤 blank NULL, \n 모두 제거
					...
					else if (cm.getCode().matches("101")) { // logout message 처리
						Logout();
						break;
					} else if (cm.getCode().matches("300")) {
						WriteAllObject(cm);
					}
					else if(cm.getCode().matches("400")) {//파일 보내 Gui 
						sendFile(cm.getFile());
					}
